...
const start = async () => {
  ...
  if ( CTZN.config.citizen.mode === 'development' ) {
    ...
    chokidar.watch([
      ...
      CTZN.config.citizen.directories.controllers + '/**.cjs',
      CTZN.config.citizen.directories.controllers + '/**.mjs' ], options).on('change', async file => {
      try {
        let extension = path.extname(file),
            fileName = path.basename(file, extension)
        helpers.log({
          label: 'Initializing ' + fileName + ' controller\n',
          content: file
        })
    ...
    chokidar.watch([
      ...
      CTZN.config.citizen.directories.models + '/**.cjs',
      CTZN.config.citizen.directories.models + '/**.mjs'], options).on('change', async file => {
      try {
        let extension = path.extname(file),
            fileName = path.basename(file, extension)
        helpers.log({
          label: 'Initializing ' + fileName + ' model\n',
          content: file
        })
        cache.clear()
        if ( CTZN.sessions && CTZN.config.citizen.development.watcher.killSession ) {
        ...
        delete require.cache[require.resolve(file)]
        CTZN.patterns.models[fileName] = await import(file + '?v=' + Math.random())
      } catch (err) {
        helpers.log({
    ...
    chokidar.watch([
      ...
      CTZN.config.citizen.directories.hooks + '/**.cjs',
      CTZN.config.citizen.directories.hooks + '/**.mjs'], options).on('change', async file => {
      try {
        let extension = path.extname(file),
            fileName = path.basename(file, extension)
        helpers.log({
        ...
        delete require.cache[require.resolve(file)]
        CTZN.hooks.app[fileName] = await import(file + '?v=' + Math.random())
      } catch (err) {
        helpers.log({
    ...
    CTZN.config.citizen.development.watcher.custom.forEach(item => {
      ...
      chokidar.watch([
        ...
        CTZN.config.citizen.directories.app + item.watch + '/**.cjs',
        CTZN.config.citizen.directories.app + item.watch + '/**.mjs'], options).on('change', async file => {
        try {
          let extension = path.extname(file),
              fileName = path.basename(file, extension)
          helpers.log({
