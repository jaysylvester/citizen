...
const start = async () => {
  ...
  if ( CTZN.config.citizen.mode === 'development' ) {
    ...
    // Route controllers
    console.log('  ' + CTZN.config.citizen.directories.routes)
    const controllerWatch = chokidar.watch(CTZN.config.citizen.directories.routes, CTZN.config.citizen.development.watcher.options)
    ...
    controllerWatch.on('change', async file => {
      try {
        ...
        CTZN.routes[fileName] = await import(file + '?v=' + cacheBuster)
        helpers.log({
    ...
    viewWatch.on('change', async file => {
      try {
        ...
        await consolidate[CTZN.config.citizen.templateEngine](CTZN.views[pattern][fileName].path, { cache: false })
        helpers.log({
