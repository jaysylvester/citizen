...
const start = async () => {
  ...
  if ( CTZN.config.citizen.log.access || CTZN.config.citizen.log.error || CTZN.config.citizen.log.debug ) {
    try {
  ...
  if ( CTZN.config.citizen.log.maxFileSize ) {
    chokidar.watch(CTZN.config.citizen.directories.logs + '/**.log', CTZN.config.citizen.development.watcher.options).on('change', file => {
      fs.stat(file, (err, stats) => {
        if ( stats.size >= CTZN.config.citizen.log.maxFileSize * 1024 ) {
