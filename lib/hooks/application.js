...
const start = async () => {
  ...
  if ( CTZN.config.citizen.logs.maxFileSize ) {
    chokidar.watch(CTZN.config.citizen.directories.logs + '/**.log', CTZN.config.citizen.logs.watcher).on('change', file => {
  ...
  if ( CTZN.config.citizen.mode === 'development' ) {
    ...
    const hookWatch = chokidar.watch(CTZN.config.citizen.directories.controllers + '/hooks', CTZN.config.citizen.development.watcher)
    ...
    const controllerWatch = chokidar.watch(CTZN.config.citizen.directories.controllers + '/routes', CTZN.config.citizen.development.watcher)
    ...
    // Helpers
    console.log('  ' + CTZN.config.citizen.directories.helpers)
    const helperWatch = chokidar.watch(CTZN.config.citizen.directories.helpers, CTZN.config.citizen.development.watcher)
    ...
    const modelWatch = chokidar.watch(CTZN.config.citizen.directories.models, CTZN.config.citizen.development.watcher)
    ...
    const viewWatch = chokidar.watch(CTZN.config.citizen.directories.views, CTZN.config.citizen.development.watcher)
    ...
    viewWatch.on('change', async file => {
      try {
        ...
        if ( CTZN.config.citizen.templateEngine !== 'templateLiterals' ) {
          ...
          try {
            await consolidate[CTZN.config.citizen.templateEngine](CTZN.views[fileName]?.path || CTZN.views[pattern][fileName].path, { cache: false })
          } catch (err) {
            helpers.log({
    ...
    CTZN.config.citizen.development.watcher.custom.forEach(item => {
      ...
      const customWatch = chokidar.watch(CTZN.config.citizen.directories.app + item.watch, CTZN.config.citizen.development.watcher)
    ...
    if ( CTZN.config.citizen.cache.static.enabled ) {
      const staticWatch = chokidar.watch(CTZN.config.citizen.directories.web, CTZN.config.citizen.development.watcher)
