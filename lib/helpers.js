...
function cache(options) {
  ...
  if ( enableCache ) {
    ...
    if ( scope !== 'controllers' && scope !== 'routes' ) {
      ...
      throw new Error('app.cache(): The terms "controllers" and "routes" are reserved cache scope names. Please choose a different name for your custom cache scope.');
    }
    ...
    if ( ( options.key && !options.file && !options.value ) || ( options.value && !options.key ) ) {
      throw new Error('app.cache(): When using a custom key, you have to specify both a key name and value.');
    }
      ...
      createEmitter = function (method) {
        ...
        emitter.on('timeout', function () {
          if ( typeof callback === 'function' ) {
            output.listen.message = 'app.listen(): ' + this.name + ' has timed out.';
