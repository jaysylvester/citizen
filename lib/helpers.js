...
function copy(object) {
  ...
  if ( Object(object) !== object || object instanceof Function ) {
    ...
    for ( var property in objectCopy ) {
      if ( Object(objectCopy[property]) !== objectCopy[property] || objectCopy[property] instanceof Function) {
        objectCopy[property] = copy(objectCopy[property]);
      }
    }
  } else {
    objectCopy = object;
  ...
  for ( var property in extension ) {
    if ( extension.hasOwnProperty(property) ) {
      if ( typeof extension[property] === 'undefined' || extension[property] === null || extension[property].constructor !== Object ) {
        ...
        mergedObject[property] = extend(mergedObject[property], copy(extension[property]));
      }
    }
  }
...
function log(options) {
  var type = options.type || 'status',
      ...
      toConsole = options.toConsole || ( CTZN.config.citizen.log.toConsole && ( ( type === 'error' && CTZN.config.citizen.log.application.errors ) || ( type === 'status' && CTZN.config.citizen.log.application.status ) ) ) || CTZN.config.citizen.mode === 'debug',
      toFile = options.toFile || ( CTZN.config.citizen.log.toFile && ( ( type === 'error' && CTZN.config.citizen.log.application.errors ) || ( type === 'status' && CTZN.config.citizen.log.application.status ) ) ),
