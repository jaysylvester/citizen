...
const log = (options) => {
  let type       = options.type || 'debug',
      ...
      toFile     = options.file || ( type === 'access' && CTZN.config.citizen.logs.access ) || ( type === 'error' && CTZN.config.citizen.logs.error ) || ( type === 'debug' && CTZN.config.citizen.logs.debug ),
...
const serverLogLabel = (statusCode, params, request) => {
  return statusCode + ' ' + http.STATUS_CODES[statusCode] + ' ' + params.route.url +  ' ' + request.remoteAddress + ' "' + request.headers['user-agent'] + '"'
}
