...
const
    ...
    compress    = {
      deflate : util.promisify(require('zlib').deflate),
      gzip    : util.promisify(require('zlib').gzip)
    },

    // citizen modules
    helpers     = require('./helpers'),
    cache       = helpers.extend(require('./cache').public, require('./cache').citizen),
    router      = require('./router'),
    session     = helpers.extend(require('./session').public, require('./session').citizen),
...
server.on('applicationStart', async (options) => {
  ...
  await CTZN.on.application.start()
  
  if ( CTZN.appOn.application && CTZN.appOn.application.start ) {
    await CTZN.appOn.application.start(CTZN.config)
  }
  createServer()
})
...
server.on('sessionStart', async (params, context) => {
  ...
  if ( !params.request.headers.origin || ( params.request.headers.origin && params.request.headers.origin.search(params.request.headers.host) ) ) {
    if ( params.cookie.ctzn_sessionID && CTZN.sessions[params.cookie.ctzn_sessionID] && CTZN.sessions[params.cookie.ctzn_sessionID].expires > Date.now() ) {
      ...
      try {
        let sessionStart = await CTZN.on.session.start(params)
        context = helpers.extend(context, sessionStart)
        if ( CTZN.appOn.session && CTZN.appOn.session.start ) {
          sessionStart = await CTZN.appOn.session.start(params)
          context = helpers.extend(context, sessionStart)
        }
        setSession(params, context)
        processRequest(params, context)
...
server.on('requestStart', async (params, context) => {
  try {
    let requestStart = await CTZN.on.request.start(params, context)
    context = helpers.extend(context, requestStart)
    
    if ( CTZN.appOn.request && CTZN.appOn.request.start ) {
      requestStart = await CTZN.appOn.request.start(params, context)
      context = helpers.extend(context, requestStart)

      // Set headers based on controller action headers
      if ( context.header ) {
        context.headerLowercase = {}
        for ( let property in context.header ) {
          if ( context.header.hasOwnProperty(property) ) {
            params.response.setHeader(property, context.header[property])
            // Create lowercase version of header name for easier comparison later
            context.headerLowercase[property.toLowerCase()] = context.header[property]
          }
        }
...
server.on('responseStart', async (params, context) => {
  try {
    let responseStart = await CTZN.on.response.start(params, context)

    context = helpers.extend(context, responseStart)
    
    if ( CTZN.appOn.response && CTZN.appOn.response.start ) {
      responseStart = await CTZN.appOn.response.start(params, context)
      context = helpers.extend(context, responseStart)
...
function response(params, context) {
  ...
  if ( routeCache ) {
    ...
    if ( routeCache.context.header ) {
      for ( var property in routeCache.context.header ) {
        if ( routeCache.context.header.hasOwnProperty(property) ) {
          params.response.setHeader(property, routeCache.context.header[property])
        }
      }
    }
...
server.on('responseEnd', async (params, context) => {
  try {
    let responseEnd = await CTZN.on.response.end(params, context)

    context = helpers.extend(context, responseEnd)
    
    if ( CTZN.appOn.response && CTZN.appOn.response.end ) {
      responseEnd = await CTZN.appOn.response.end(params, context)
      context = helpers.extend(context, responseEnd)
...
server.on('error', async (err, params, context) => {
  ...
  helpers.log({
    type    : 'error',
    label   : label + ( statusCode === 404 ? ': ' + params.route.url : '\n'),
  ...
  if ( statusCode !== 404 ) {
    try {
      await CTZN.on.application.error(err, params, context)
      if ( CTZN.appOn.application && CTZN.appOn.application.error ) {
        await CTZN.appOn.application.error(err, params, context)
      }
    } catch ( err ) {
      if ( CTZN.config.citizen.mode === 'production' || !debug(params, context) ) {
        logContent = err.stack ? requested + err.stack : requested + util.inspect(err)
  ...
  if ( params && !params.response.finished ) {
    ...
    if ( cachedFile ) {
      ...
      fs.readFile(staticPath, function (err, data) {
        if ( !err ) {
          ...
          helpers.log({
            type  : 'error',
            label : response.statusCode + ' ' + http.STATUS_CODES[response.statusCode] + ': ' + params.route.filePath
          })
        }
      })
    }
    ...
    if ( context.session.expires && context.session.expires === 'now' ) {
      ...
      for ( var property in context.session ) {
        if ( context.session.hasOwnProperty(property) && ( CTZN.reserved.session.indexOf(property) < 0 || system ) ) {
          CTZN.sessions[params.session.id][property] = context.session[property]
        } else {
  ...
  if ( context.redirect && ( typeof context.redirect === 'string' || helpers.size(context.redirect) ) && params.route.type !== 'direct' ) {
    ...
    if ( params.request.headers.origin ) {
      ...
      if ( !corsOriginTest.test(params.request.headers.origin) ) {
        if ( controller.config && controller.config.cors && controller.config.cors[params.route.action] ) {
          ...
          for ( let property in controller.config.cors[params.route.action] ) {
            if ( controller.config.cors[params.route.action].hasOwnProperty(property) ) {
              // Create lowercase version of header name for easier comparison, while maintaining case for sent header
              controller.config.corsLowerCase[params.route.action][property.toLowerCase()] = controller.config.cors[params.route.action][property]
            }
          }
          if ( controller.config.corsLowerCase[params.route.action]['access-control-allow-origin'] && ( controller.config.corsLowerCase[params.route.action]['access-control-allow-origin'].search(params.request.headers.origin) >= 0 || controller.config.corsLowerCase[params.route.action]['access-control-allow-origin'] === '*' ) ) {
            if ( params.request.method === 'OPTIONS' && !params.request.headers['access-control-request-method'] ) {
              ...
              for ( let property in controller.config.cors[params.route.action] ) {
                if ( controller.config.cors[params.route.action].hasOwnProperty(property) ) {
                  params.response.setHeader(property, controller.config.cors[params.route.action][property])
                }
              }
    ...
    if ( respond ) {
      switch ( params.request.method ) {
        case 'GET':
          try {
            let requestEnd = await CTZN.on.request.end(params)
            context = helpers.extend(context, requestEnd)
            if ( CTZN.appOn.request && CTZN.appOn.request.end ) {
              requestEnd = await CTZN.appOn.request.end(params)
              context = helpers.extend(context, requestEnd)
            }
            setSession(params, context)
...
async function fireController(params, context) {
  ...
  params.response.on('close', function () {
    params.response.end()
    helpers.log({
      label: 'Connection closed by client'
    })
  })
  ...
  try {
    pattern = cachedController ? cachedController.context || {} : await CTZN.patterns.controllers[controller][action](params) || {}
  } catch (err) {
    proceed = false
    server.emit('error', err, params, context)
  }
  ...
  if ( proceed ) {
    ...
    if ( !params.response.finished ) {

      for ( let property in context.content ) {
...
function handoff(params, context, thisHandoff, action, renderedView, renderer) {
  ...
  helpers.log({
    label: 'Handing off to the ' + thisHandoff.controller + ' controller'
  })
  fireController(params, context)
}
...
function debug(params, context) {
  ...
  try {
    if ( toDebug === 'context' ) {
      ...
      label = toDebug + ' dump'
      dump = Function('"use strict";return (' + toDebug + ')')()
    }
    helpers.log({
      label       : label,
      content     : dump,
    ...
    if ( CTZN.config.citizen.development.debug.view ) {
      viewDump = dump.replace(/</g, '&lt;')
