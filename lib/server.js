...
function createServer(context) {
        ...
        fs.readFile(staticPath, function (err, data) {
          if ( err ) {
            ...
            if ( CTZN.config.citizen.mode !== 'production' ) {
              console.log('404 Not Found: ' + params.route.filePath);
            }
          } else {
  ...
  // }).listen(CTZN.config.citizen.httpPort, CTZN.config.citizen.hostname, CTZN.config.citizen.backlog);
  }).listen(CTZN.config.citizen.httpPort, CTZN.config.citizen.hostname, CTZN.config.citizen.backlog);
}
  ...
  responseDomain.run( function () {
    listen({
      ...
      var requestContext = helpers.extend(context, output.pattern),
          handoff = helpers.copy(requestContext.handoff),
          handoffController = {},
          handoffParams = {},
          ...
          renderedView = handoff.view || handoff.controller || requestContext.view || renderer,
          contentType;
      ...
      if ( output.listen.success ) {
        ...
        if ( CTZN.config.citizen.sessions && ( !params.request.headers.origin || ( params.request.headers.origin && params.request.headers.origin.search(params.request.headers.host) ) ) && Object.getOwnPropertyNames(requestContext.session).length > 0 ) {
          if ( requestContext.session.expires && requestContext.session.expires === 'now' ) {
            ...
            requestContext.cookie = helpers.extend(requestContext.cookie, { ctzn_session_id: { expires: 'now' }});
            sessionEnd(helpers.copy(params), helpers.copy(requestContext));
          } else {
            CTZN.sessions[params.session.id] = helpers.extend(CTZN.sessions[params.session.id], requestContext.session);
          }
        }
        ...
        if ( requestContext.redirect.url ) {
          ...
          if ( handoff.controller ) {
            handoffController = CTZN.patterns.controllers[handoff.controller];
            handoffParams = helpers.extend(params, { route: { renderer: renderer, renderedView: renderedView }});
            ...
            if ( includeProperties.length > 0 && params.url.type !== 'ajax' ) {
              includeProperties.forEach( function (item, index, array) {
                includeGroup[item] = function (emitter) {
                  CTZN.patterns.controllers[item].handler(helpers.copy(params), helpers.copy(requestContext), emitter);
                };
              });
              listen(includeGroup, function (output) {
                includeProperties.forEach( function (item, index, array) {
                  requestContext.content = helpers.extend(requestContext.content, output[item].content);
                });
...
function respond(params, requestContext, renderer, renderedView, include, includeProperties) {
  var contentType,
      viewContext;
  ...
  switch ( params.route.format ) {
    case 'html':
      ...
      viewContext.include = {};
      includeProperties.forEach( function (item, index, array) {
        viewContext.include[item] = renderView(item, include[item], 'html', viewContext);
      });
      if ( params.route.controller !== params.route.renderer && CTZN.patterns.views[params.route.controller] && CTZN.patterns.views[params.route.controller][params.route.view] ) {
...
function renderView(pattern, view, format, context) {
  ...
  if ( CTZN.patterns.views[pattern][view] ) {
    ...
    throw {
      ...
      // This is a 500 instead of a 404 because the controller exists, meaning the URL is valid
      statusCode: 500,
      message: 'The requested view doesn\'t exist.'
    };
  }
}
