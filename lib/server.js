  ...
  requestDomain.run( function () {
    ...
    if ( !params.route.isStatic ) {
      ...
      if ( CTZN.config.citizen.cache.static ) {
        cachedFile = helpers.retrieve({ file: CTZN.config.citizen.directories.web + params.route.filePath });
      }
      ...
      if ( cachedFile ) {
        ...
        if ( CTZN.config.citizen.gzip.enable && params.client.encoding === 'gzip' ) {
          ...
          response.write(cachedFile.gzip);
        } else {
          response.write(cachedFile.identity);
        }
          ...
          if ( !err ) {
            ...
            if ( CTZN.config.citizen.gzip.enable && CTZN.config.citizen.gzip.mimeTypes.indexOf(CTZN.config.citizen.mimetypes[params.route.extension]) >= 0 ) {
              zlib.gzip(data, function (err, zippedFile) {
                ...
                if ( CTZN.config.citizen.cache.static ) {
                  helpers.cache({
                    file: CTZN.config.citizen.directories.web + params.route.filePath,
                    value: {
                      identity: data,
                      gzip: zippedFile
                    }
  ...
  if ( CTZN.config.citizen.gzip.enable && params.client.encoding === 'gzip' ) {
    zlib.gzip(view, function (err, zippedView) {
      if ( !err ) {
        ...
        params.response.end();
        server.emit('responseEnd', params, context);
        cacheResponse(params, context, view, zippedView, contentType);
      } else {
        ...
        params.response.end();
        server.emit('responseEnd', params, context);
        cacheResponse(params, context, view, '', contentType);
        throw new Error(err);
      }
    });
  } else {
...
function cacheResponse(params, context, view, zippedView, contentType) {
  if ( context.cache ) {
