...
server.on('request', function (params, request, response, context) {
  ...
  if ( context.redirect && ( typeof context.redirect === 'string' || Object.keys(context.redirect).length ) && !params.route.direct ) {
    ...
    // If the controller doesn't exist, throw a 404
      ...
      let err = new Error()
      err.statusCode = 404
      // Remove the non-existent controller from the chain
      delete params.route.chain[controllerName]
      server.emit('error', params, request, response, context, err)
    }
  }
})
              ...
              if ( request.headers['if-none-match'] == lastModified ) {
                ...
                if ( CTZN.config.citizen.compression.enable && compressable ) {
                  let [
                    ...
                    CTZN.config.citizen.cache.static.enable || request.client.encoding === 'gzip' ? compress.gzip(data) : false,
                    CTZN.config.citizen.cache.static.enable || request.client.encoding === 'deflate' ? compress.deflate(data) : false
...
function serverResponse(params, request, response, context) {
  ...
  if ( routeCache ) {
    ...
    if ( request.headers['if-none-match'] == routeCache.lastModified ) {
      ...
      response.setHeader('Content-Encoding', request.client.encoding)
      response.end(routeCache.output[request.client.encoding])
    }
    server.emit('responseEnd', params, request, response, context)
  } else {
...
async function fireController(params, request, response, context) {
  let controller = context.handoff ? context.handoff.controller : params.route.controller,
      ...
      view = context.handoff ? context.handoff.view : context.view || params.route.view,
      controllerParams = helpers.copy(params),
      patternCache = cache.getController({ controller: controller, action: action, view: view, route: 'global', contentType: response.contentType }) || cache.getController({ controller: controller, action: action, view: view, route: params.route.pathname, contentType: response.contentType }),
      include = {},
      includeProperties,
      includes = [],
      pattern = {},
  ...
  if ( patternCache ) {
    pattern = patternCache.context || {}
    params.route.chain[controller] = patternCache
  ...
  if ( proceed ) {
    ...
    context.public = context.public || {}
    if ( response.contentType === 'text/html' && !context.handoff && CTZN.config.citizen.layout.controller.length && controller !== CTZN.config.citizen.layout.controller ) {
      context.handoff = CTZN.config.citizen.layout
    }

    params.route.chain[controller].params = controllerParams
    params.route.chain[controller].context = helpers.copy(context)
    params.route.chain[controller].view = context.view || params.route.chain[controller].view
        ...
        if ( params.config.citizen.contentTypes.indexOf(response.contentType) >= 0 ) {
          ...
          if ( proceed ) {
            if ( context.redirect && ( typeof context.redirect === 'string' || ( Object.keys(context.redirect).length && typeof context.redirect.refresh === 'undefined' ) ) && !params.route.direct ) {
              ...
              if ( includeProperties.length && response.contentType === 'text/html' ) {
                params.route.chain[controller].public = params.route.chain[controller].public || {}
                params.route.chain[controller].public.include = {}
                includeProperties.forEach( function (item, index) {
                  ...
                  include[item].cache = cache.getController({ controller: include[item].controller, action: include[item].action, view: include[item].view, route: 'global', contentType: response.contentType }) || cache.getController({ controller: include[item].controller, action: include[item].action, view: include[item].view, route: include[item].pathname, contentType: response.contentType })
    
                  context.public.include = context.public.include || {}
      
                  includes[index] = ( async () => {
                    if ( include[item].cache ) {
                      params.route.chain[controller].public.include[item] = include[item].cache
                    } else {
                      helpers.log({
                      ...
                      try {
                        ...
                        params.route.chain[controller].public.include[item] = {
                          controller : include[item].controller,
                          action     : include[item].action,
          ...
          let err = new Error(response.contentType + ' output is not available for this resource. Use the global or controller config to enable ' + response.contentType + ' output.')
          err.statusCode = 406
          server.emit('error', params, request, response, context, err)
        }
      }
...
async function respond(params, request, response, context) {
  ...
  for ( const link of Object.keys(params.route.chain) ) {
    ...
    if ( params.route.chain[link].context.view !== false ) {
      ...
      if ( params.route.chain[link].public && params.route.chain[link].public.include ) {
        params.route.chain[link].context.public.include = {}
        for ( const include of Object.keys(params.route.chain[link].public.include) ) {
          ...
          includes[includeInstance] = ( async () => {
            ...
            if ( params.route.chain[link].public.include[include].output ) {
              helpers.log({
                label: 'Using cached include controller rendering: ' + include,
                content: {
                  controller  : params.route.chain[link].public.include[include].controller,
                  action      : params.route.chain[link].public.include[include].action,
                  view        : params.route.chain[link].public.include[include].view
                }
              })
              return params.route.chain[link].public.include[include].output
            } else {
              let publicContext
              try {
                publicContext = setPublicContext(params, context, response.contentType, params.route.chain[link].public.include[include].context.public, params.route.chain[link].public.include[include].params)
              } catch ( err ) {
                return Promise.reject(err)
              }
              ...
              let output = await renderView(params, request, response, context, {
                pattern       : params.route.chain[link].public.include[include].controller,
                view          : params.route.chain[link].public.include[include].view,
                params        : params.route.chain[link].public.include[include].params,
              ...
              if ( CTZN.config.citizen.cache.application.enable ) {
                try {
                  cacheController({
                    ...
                    action        : params.route.chain[link].public.include[include].action,
                    view          : params.route.chain[link].public.include[include].view,
                    route         : params.route.chain[link].public.include[include].params.route.pathname,
                    contentType   : response.contentType,
                    params        : params.route.chain[link].public.include[include].params,
                    context       : params.route.chain[link].public.include[include].context,
                    output        : output
                  })
                } catch ( err ) {
                  return Promise.reject(err)
      ...
      await Promise.allSettled(includes).then( async (results) => {
        results.forEach( (item, index) => {
          params.route.chain[link].context.public.include[includeNames[index]] = item.value
        })
        if ( params.route.chain[link].output ) {
          ...
          let publicContext
          try {
            publicContext = setPublicContext(params, context, response.contentType, params.route.chain[link].context.public, params.route.chain[link].params)
          } catch ( err ) {
  ...
  try {
    ...
    if ( CTZN.config.citizen.compression.enable && ( ( context.cache && context.cache.route ) || ( request.client.encoding === 'gzip' || request.client.encoding === 'deflate' ) ) ) {
      let [
        deflated,
        zipped
      ] = await Promise.all([
        context.cache || request.client.encoding === 'deflate' ? compress.deflate(output) : '',
        context.cache || request.client.encoding === 'gzip'    ? compress.gzip(output)    : ''
      ]).catch(err => { server.emit('error', params, request, response, context, err) })
    ...
    if ( !response.writableEnded ) {
      response.setHeader('Content-Encoding', request.client.encoding)
      response.end(encodings[request.client.encoding])
      server.emit('responseEnd', params, request, response, context)
    }

    if ( CTZN.config.citizen.cache.application.enable ) {
      cacheResponse(params, request, response, context, encodings.identity, encodings.gzip, encodings.deflate, lastModified)
    }
  } catch (err) {
    server.emit('error', params, request, response, context, err)
  }
...
function setPublicContext(params, context, contentType, controllerContext, controllerParams) {
  let publicContext = {}
  ...
  switch ( contentType ) {
    case 'text/html':
      ...
      if ( CTZN.config.citizen.mode === 'development' && ( CTZN.config.citizen.development.debug.view || params.url.ctzn_debug ) ) {
        publicContext.ctzn_debugOutput = debug(params, context)
      }
      break
    case 'application/json':
      if ( !params.url.output ) {
        publicContext = context.public
      } else {
        publicContext = createJSON(params, context)
      }
      break
    case 'application/javascript':
      if ( !params.url.output ) {
...
function cacheResponse(params, request, response, context, identity, zipped, deflated, lastModified) {
  let cacheExists = cache.exists({
        route: params.route.pathname,
        contentType: response.contentType
      }),
...
function createJSON(params, context) {
  let outputArray = params.url.output.split(','),
      outputNode,
      output = {},
      failed = () => {
        let err = new Error('The requested JSON notation ("' + params.url.output + '") doesn\'t exist. Make sure you\'re using a comma (,) as a delimiter for nested output.')
        err.statusCode = 404
        // Remove output URL parameter or respond() loops infinitely
        delete params.url.output
        throw err
      }

  if ( context.public[decodeURIComponent(outputArray[0])] ) {
    output = context.public[decodeURIComponent(outputArray[0])]
...
function debug(params, context) {
  ...
  try {
    ...
    // Dump the debug output to the view if specified
