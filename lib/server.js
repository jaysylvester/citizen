      ...
      if ( handoff.controller ) {
        ...
        if ( Object.getOwnPropertyNames(include).length > 0 && params.url.type !== 'ajax' ) {
          ...
          listen(includeGroup, function (output) {
            ...
            // TODO: This is only necessary if the original requested view is to be rendered as a partial itself.
            // Need a switch to skip this somehow. Probably could compare params.route.name to params.route.renderer,
            // which is modified above in the case of a handoff.
