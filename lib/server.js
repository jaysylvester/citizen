...
server.on('request', function (params, request, response, context) {
  ...
  if ( context.redirect && ( typeof context.redirect === 'string' || Object.keys(context.redirect).length ) && !params.route.direct ) {
    ...
    if ( CTZN.controllers.routes[controllerName] && CTZN.controllers.routes[controllerName][params.route.action] ) {
...
server.on('error', async (params, request, response, context, err, respond = true) => {
  ...
  if ( statusCode >= 500 ) {
    try {
      await applicationHooks.error(params, request, response, context, err)
    } catch ( err ) {
      helpers.log({
        type    : 'error:server',
  ...
  if ( params && !response.writableEnded ) {
    ...
    if ( respond ) {
      let controller = statusCode !== 404 ? params.route.chain[Object.keys(params.route.chain)[Object.keys(params.route.chain).length-1]].controller : 'error',
          error = {
            errorCode  : err.code,
      ...
      switch ( response.contentType ) {
        ...
        case 'text/plain':
          ...
          if ( CTZN.views.error ) {
            ...
            if ( params.config.citizen.layout.controller.length && CTZN.controllers.routes[params.config.citizen.layout.controller] && controller !== params.config.citizen.layout.controller ) {
...
function setCookies(params, request, response, context) {
  if ( context.cookies ) {
    let cookie = buildCookie(params, request, context.cookies)
...
async function fireController(params, request, response, context) {
  ...
  if ( proceed ) {
    ...
    delete context.headers
    delete context.headersLowercase
            ...
            if ( includeProperties.length ) {
              ...
              includeProperties.forEach( function (item, index) {
                ...
                includes[index] = ( async () => {
                  ...
                  include[item].cache = cache.getRoute({
                  ...
                  if ( params.route.chain[controller].include[item].context.local ) {
...
async function respond(params, request, response, context) {
        ...
        for ( const include of Object.keys(params.route.chain[link].include) ) {
          ...
          includes[includeInstance] = ( async () => {
            ...
            if ( params.route.chain[link].include[include].output ) {
              helpers.log({
                label: 'Using cached include controller action: ' + include,
                content: {
                  route       : params.route.chain[link].include[include].route,
                  contentType : params.route.chain[link].include[include].contentType
                }
              })
              ...
              try {
                let renderedView = await renderView(params, request, response, context, {
                  controller    : params.route.chain[link].include[include].controller,
                  view          : params.route.chain[link].include[include].view,
                ...
                // Only cache the include if the view rendered successfully
                if ( renderedView.success && CTZN.config.citizen.cache.application.enabled && params.route.chain[link].include[include].context.cache?.action ) {
                  cacheRoute(params, request, response, context, {
                    params    : params.route.chain[link].include[include].params,
                    context   : params.route.chain[link].include[include].context,
                    output    : renderedView.output,
                    encodings : await encodeOutput(params, request, response, params.route.chain[link].include[include].context, renderedView.output)
                  })
                }

                // Bypass caching for the calling controller if renderView() throws an error
                if ( !renderedView.success ) {
                  delete params.route.chain[link].context.cache
                }

                return renderedView.output
              } catch ( err ) {
                return Promise.reject(err)
              }
            }
      ...
      await Promise.all(includes).then( async (results) => {
        ...
        if ( results.length && response.contentType !== 'application/json' && response.contentType !== 'application/javascript' ) {
          results.forEach( (item, index) => {
            params.route.chain[link].context.include[includeNames[index]] = item
          })
        }
        if ( params.route.chain[link].output ) {
          helpers.log({
            label: 'Using cached controller action: ' + link,
            content: {
              route       : params.route.chain[link].route,
              contentType : params.route.chain[link].contentType
            }
          })
        } else {
          let publicContext
          ...
          try {
            ...
            params.route.chain[link].output = renderedView.output

            // Only cache the rendered view if it was rendered successfully. Don't cache the layout controller.
            if ( renderedView.success && CTZN.config.citizen.cache.application.enabled && params.route.chain[link].context.cache?.action && params.route.chain[link].controller !== CTZN.config.citizen.layout.controller ) {
              cacheRoute(params, request, response, context, {
                params    : params.route.chain[link].params,
                context   : params.route.chain[link].context,
                output    : params.route.chain[link].output,
                encodings : await encodeOutput(params, request, response, params.route.chain[link].context, params.route.chain[link].output)
              })
            }
        ...
        // Stop client rendering and let the error handler deal with it
        proceed = false
        server.emit('error', params, request, response, context, err)
      })
    }
  ...
  if ( proceed ) {
    try {
      ...
      // If it's JSON, roll up all local contexts in the chain into the final output.
      if ( Object.keys(params.route.chain).length > 1 && ( response.contentType === 'application/javascript' || response.contentType === 'application/json' ) ) {
        let routeContext = {
          local: {}
        }
        
        Object.keys(params.route.chain).map( controller => {
          try {
            return {
              controller: controller,
              output: JSON.parse(params.route.chain[controller].output)
            }
          } catch {
            return {
              controller: controller,
              output: params.route.chain[controller].output
            }
          }
        }).forEach( function (item) {
          routeContext.local[item.controller] = item.output
        })

        let publicContext = setPublicContext(params, request, response, context, routeContext, params.route.chain[Object.keys(params.route.chain)[0]].params)
        
        let renderedView = await renderView(params, request, response, context, {
          controller    : params.route.chain[Object.keys(params.route.chain)[0]].controller,
          view          : params.route.chain[Object.keys(params.route.chain)[0]].view,
          params        : params.route.chain[Object.keys(params.route.chain)[0]].params,
          context       : publicContext
        })

        encodings = await encodeOutput(params, request, response, context, renderedView.output)
      // The last controller in the chain provides the final rendered view.
      } else {
        encodings = await encodeOutput(params, request, response, context, params.route.chain[Object.keys(params.route.chain)[Object.keys(params.route.chain).length-1]].output)
      }
      ...
      if ( CTZN.config.citizen.cache.application.enabled && params.route.chain[Object.keys(params.route.chain)[0]].context.cache?.request ) {
        context.cache = context.cache || {}
        context.cache.request = params.route.chain[Object.keys(params.route.chain)[0]].context.cache.request

        cacheRoute(params, request, response, context, {
          params    : params,
          context   : context,
          output    : params.route.chain[Object.keys(params.route.chain)[Object.keys(params.route.chain).length-1]].output,
          encodings : encodings
        })
      }
    } catch (err) {
      ...
      urlParamCheck = function (urlParams) {
        if ( urlParams && Object.keys(options.params.url).length ) {
          Object.keys(options.params.url).forEach( function (item) {
            if ( item !== params.route.controller && urlParams.indexOf(item) < 0 && CTZN.reserved.url.indexOf(item) < 0 ) {
              proceed = false
...
async function renderView(params, request, response, context, options) {
  let viewContext = typeof options.context === 'string' ? options.context : Object.assign({}, options.context), // consolidate modifies the original, so copy it
      json = ''
        ...
        try {
          ...
          if ( CTZN.config.citizen.templateEngine === 'templateLiterals' ) {
            ...
            return await import('consolidate').then((consolidate) => {
              html = consolidate[CTZN.config.citizen.templateEngine](CTZN.views[options.view]?.path || CTZN.views[options.controller][options.view].path, viewContext)
              if ( options.context.ctzn_debugOutput ) {
                html = html.replace('</body>', '\n<div id="citizen-debug">\n' + options.context.ctzn_debugOutput + '\n</div>\n</body>')
              }
