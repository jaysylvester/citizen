...
server.on('error', async (params, request, response, context, err, respond = true) => {
  ...
  let label = statusCode + ' ' + http.STATUS_CODES[statusCode] + ( err.message ? ': ' + err.message : '' )
  ...
  if ( statusCode >= 500 ) {
    if ( CTZN.config.citizen.mode === 'production' || !debug(params, request, response, context) ) {
      ...
      logContent = err.stack ? debug(params, request, response, context) + ' ' + err.stack : debug(params, request, response, context) + ' ' + util.inspect(err)
    }
  }

  if ( statusCode === 404 && request.headers.referer ) {
