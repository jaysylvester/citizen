...
server.on('request', function (params, request, response, context) {
  ...
  if ( context.redirect && ( typeof context.redirect === 'string' || Object.keys(context.redirect).length ) && !params.route.direct ) {
    ...
    if ( CTZN.routes[controllerName] && CTZN.routes[controllerName][params.route.action] ) {
...
async function fireController(params, request, response, context) {
  ...
  // Clear the previous controller's next and cache directives
  delete context.next
  ...
  if ( !params.route.chain[controller] ) {
    ...
    try {
      params.route.chain[controller] = {
        controller : controller,
        action     : action,
        context    : await CTZN.routes[controller][action](controllerParams, request, response, context) || {}
      }
    } catch ( err ) {
      proceed = false
      server.emit('error', params, request, response, context, err)
            ...
            if ( includeProperties.length ) {
              ...
              includeProperties.forEach( function (item, index) {
                ...
                includes[index] = ( async () => {
                  ...
                  // If the include is defined using the controller/action syntax
                  if ( include[item].controller ) {
                    ...
                    include[item].params.route.pathname = '/' + include[item].controller + ( include[item].action !== 'handler' ? '/action/' + include[item].action : '' )
                  // If the include is defined using the route pathname syntax
                  } else {
                    try {
                      ...
                      err.message = 'The requested include route (' + include[item].route + ') is invalid. When using the route syntax to specify an include, the route must be a valid pathname starting with a forward slash (/).'
                  ...
                  if ( include[item].cache ) {
                    ...
                    try {
                      include[item].context = await CTZN.routes[include[item].controller][include[item].action](include[item].params, request, response, context) || {}
                      ...
                      } else if ( !CTZN.routes[include[item].controller] ) {
                        err.message = 'The controller you requested to be included (' + include[item].controller + ') doesn\'t exist.'
                      } else if ( !CTZN.routes[include[item].controller][include[item].action] ) {
...
function next(params, request, response, context) {
  if ( !response.writableEnded ) {
    if ( context.next && !params.route.direct ) {
      if ( context.next.route ) {
        context.next.params = {}
        context.next.params.route = router.parseRoute(request, 'http', context.next.route)
        context.next.params.url = context.next.params.route.urlParams
      }
      ...
      context.next.view = context.next.params?.route.view || context.next.view || context.next.controller

      delete context.view

      helpers.log({
        label: 'Handing off to controller: ' + context.next.controller
      })

      if ( CTZN.routes[context.next.controller] ) {
        ...
        let err = new Error('The specified next controller (' + context.next.controller + ') doesn\'t exist.')
        server.emit('error', params, request, response, context, err)
      }
    } else {
...
async function respond(params, request, response, context) {
          ...
          includes[includeInstance] = ( async () => {
            ...
            if ( params.route.chain[link].include[include].output ) {
              ...
              try {
                ...
                if ( CTZN.config.citizen.cache.application.enabled && params.route.chain[link].include[include].context.cache?.action ) {
                  cacheRoute(params, request, response, context, {
                    params    : params.route.chain[link].include[include].params,
                    context   : params.route.chain[link].include[include].context,
                    output    : output,
                    encodings : await encodeOutput(params, request, response, params.route.chain[link].include[include].context, output)
                  })
                }
      ...
      await Promise.all(includes).then( async (results) => {
        ...
        if ( params.route.chain[link].output ) {
          ...
          try {
            ...
            // Don't cache the layout controller
            if ( CTZN.config.citizen.cache.application.enabled && params.route.chain[link].context.cache?.action && params.route.chain[link].controller !== CTZN.config.citizen.layout.controller ) {
              cacheRoute(params, request, response, context, {
                params    : params.route.chain[link].params,
  ...
  try {
    ...
    let encodings = await encodeOutput(params, request, response, context, params.route.chain[Object.keys(params.route.chain)[Object.keys(params.route.chain).length-1]].output)
      ...
      urlParamCheck = function (urlParams) {
        if ( urlParams && Object.keys(options.params.url).length ) {
          Object.keys(options.params.url).forEach( function (item) {
            if ( urlParams.indexOf(item) < 0 && CTZN.reserved.url.indexOf(item) < 0 ) {
              proceed = false
              let err = new Error('Cache attempt on ' + route + ' failed due to an invalid URL parameter (' + item + '). Request denied.')
              server.emit('error', params, request, response, context, err)
            }
          })
        }
...
function extendConfig(params, controller, action) {
  if ( CTZN.routes[controller].config ) {
    // Controller config
    if ( CTZN.routes[controller].config.controller ) {
      params.config.citizen = helpers.extend(params.config.citizen, CTZN.routes[controller].config.controller)
    }
    // Controller action config
    if ( CTZN.routes[controller].config[action] ) {
      params.config.citizen = helpers.extend(params.config.citizen, CTZN.routes[controller].config[action])
    }
  }
}
