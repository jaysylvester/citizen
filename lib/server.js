...
function createServer() {
  ...
  serverDomain.on('error', function (err) {
    var localUrl = CTZN.config.citizen.httpPort === 80 ? 'http://localhost' : 'http://localhost:' + CTZN.config.citizen.httpPort;
    ...
    switch ( err.code ) {
      ...
      case 'ENOTFOUND':
        helpers.log({
          content: '\nServer startup failed because the hostname you specified in your config file ("' + CTZN.config.citizen.hostname + '") isn\'t available.\n\nTry running citizen without specifying a hostname (accessible via ' + localUrl + ' locally or your server\'s IP address remotely). If that works, then the issue is probably in your server\'s DNS settings.\n',
          toConsole: true,
          file: 'citizen.txt',
          timestamp: false
        });
