...
async function fireController(params, request, response, context) {
  ...
  // Clear the previous controller's handoff and cache directives
...
function handoff(params, request, response, context) {
  if ( !response.writableEnded ) {
    if ( context.handoff && !params.route.direct ) {
      ...
      if ( CTZN.routes[context.handoff.controller] ) {
          ...
          includes[includeInstance] = ( async () => {
            ...
            if ( params.route.chain[link].include[include].output ) {
              ...
              try {
                ...
                if ( CTZN.config.citizen.cache.application.enabled && params.route.chain[link].include[include].context.cache?.controller ) {
      ...
      await Promise.all(includes).then( async (results) => {
        ...
        if ( params.route.chain[link].output ) {
          ...
          try {
            ...
            if ( CTZN.config.citizen.cache.application.enabled && params.route.chain[link].context.cache?.controller && params.route.chain[link].controller !== CTZN.config.citizen.layout.controller ) {
              cacheRoute(params, request, response, context, {
                params    : params.route.chain[link].params,
...
function cacheRoute(params, request, response, context, options) {
  ...
  if ( options.context.cache?.controller ) {
    route         = options.params.route.pathname
    lastModified  = options.context.cache.controller.lastModified
    lifespan      = options.context.cache.controller.lifespan
    resetOnAccess = options.context.cache.controller.resetOnAccess

    urlParamCheck(options.context.cache.controller.urlParams)
  // Cache the request
