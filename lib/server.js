...
async function fireController(params, request, response, context) {
  let controller = context.handoff ? context.handoff.controller : params.route.controller,
      ...
      patternCache = cache.getController({ controller: controller, action: action, view: view, route: 'global' }) || cache.getController({ controller: controller, action: action, view: view, route: params.route.pathname }),
      include = {},
      includeProperties,
      includes = [],
      pattern = {},
...
function cacheResponse(params, request, response, context, render, zipped, deflated, lastModified) {
  ...
  if ( context.cache && context.cache.route && !cache.exists({ route: params.route.pathname }) ) {
