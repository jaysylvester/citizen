...
server.on('request', function (params, request, response, context) {
  ...
  if ( context.redirect && ( typeof context.redirect === 'string' || Object.keys(context.redirect).length ) && !params.route.direct ) {
    ...
    if ( CTZN.patterns.controllers[controllerName] && CTZN.patterns.controllers[controllerName][params.route.action] ) {
      ...
      extendConfig(params, CTZN.patterns.controllers[controllerName])
      ...
      if ( request.headers.origin && !/^ + params.route.base + $/.test(request.headers.origin) ) {
...
async function processRequest(params, request, response, context) {
  ...
  const maxPayloadExceeded = (clientError) => {
  ...
  switch ( request.method ) {
    ...
    case 'PUT':
      ...
      // Extend the config with the controller config
      extendConfig(params, CTZN.patterns.controllers[params.route.controller])

      if ( request.headers['content-length'] < CTZN.config.citizen.forms.maxPayloadSize ) {
        // Concatenate chunks to build the full payload. Encode as binary to preserve
        // images in multipart form data.
        let stringDecoder = new StringDecoder('binary')

        request.on('data', (chunk) => {
          if ( payload.length < CTZN.config.citizen.forms.maxPayloadSize ) {
