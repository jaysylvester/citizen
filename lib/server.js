...
server.on('error', function (err, request, response, params, context) {
  ...
  helpers.log({
    label: context && context.error && context.error.thrownBy ? 'Error thrown by ' + context.error.thrownBy : 'Server error',
    content: context && context.error ? context.error : err.stack ? err.stack : err,
    file: 'citizen.txt'
  });
  ...
  if ( !response.headersSent ) {
    ...
    switch ( params.route.format ) {
      ...
      case 'jsonp':
        ...
        response.write(renderView('', '', params.route.format, context, params.url.callback));
        break;
    }
    ...
    if ( !params.route.isStatic ) {
      ...
      if ( CTZN.patterns.controllers[CTZN.config.citizen.fallbackController || params.route.controller] && CTZN.patterns.controllers[CTZN.config.citizen.fallbackController || params.route.controller][params.route.action] ) {
        ...
        context.error = {
          ...
          statusCode: 404,
          message: 'The requested controller action doesn\'t exist.\n\nController: ' + params.route.controller + '\nAction:' + params.route.action + '()'
        };
        throw new Error('404 - The requested controller action doesn\'t exist.\n\nController: ' + params.route.controller + '\nAction: ' + params.route.action + '()\n\n');
      }
    } else {
      ...
      if ( CTZN.config.citizen.cache.static ) {
        cachedFile = helpers.retrieve({ file: staticPath, output: 'all' });
      }
      ...
      if ( cachedFile ) {
        ...
        fs.readFile(staticPath, function (err, data) {
          if ( !err ) {
            ...
            throw new Error('404 Not Found: params.route.filePath');
          }
        });
      }
  ...
  responseDomain.run( function () {
    ...
    helpers.listen({
      ...
      if ( output.listen.success ) {
        ...
        if ( requestContext.redirect && ( typeof requestContext.redirect === 'string' || ( helpers.size(requestContext.redirect) && typeof requestContext.redirect.refresh === 'undefined' ) ) && params.route.type !== 'direct' ) {
          ...
          if ( requestContext.redirect && ( typeof requestContext.redirect === 'string' || helpers.size(requestContext.redirect) ) && params.route.type !== 'direct' ) {
            redirect(params, requestContext);
          }
          ...
          if ( includeProperties.length && params.route.format === 'html' ) {
            includeProperties.forEach( function (item, index, array) {
              ...
              include[item].cache = helpers.retrieveController({ controller: include[item].controller, action: include[item].action, view: include[item].view, route: include[item].pathname }) || helpers.retrieveController({ controller: include[item].controller, action: include[item].action, view: include[item].view, route: 'global' });

              includeGroup[item] = function (emitter) {
                if ( include[item].cache ) {
                  emitter.emit('ready', include[item].cache.context);
                } else {
                  CTZN.patterns.controllers[include[item].controller][include[item].action](include[item].params, requestContext, emitter);
                }
              };
            });
            ...
            if ( helpers.size(includeGroup) ) {
              ...
              helpers.listen(includeGroup, function (output) {
                includeProperties.forEach( function (item, index, array) {
                  ...
                  if ( include[item].cache ) {
                    requestContext.content.include[item] = include[item].cache.render;
                    helpers.log({
                    ...
                    requestContext.content.include[item] = include[item].render;
                    helpers.log({
                ...
                if ( requestContext.handoff && params.route.type !== 'direct' ) {
        ...
        error = new Error(output.listen.message || http.STATUS_CODES[output.listen.statusCode || 500]);

        output.listen.stack = output.listen.stack || error.stack;

        server.emit('error', output.listen, params.request, params.response, params, requestContext);
      }
    });
  });
}
...
function handoff(params, requestContext) {
  ...
  if ( requestContext.cache ) {
    delete requestContext.cache.controller;
  }
