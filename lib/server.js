...
function serve(request, response, protocol) {
  ...
  requestDomain.on('error', function (err, request, response, params, context) {
    if ( !err.staticAsset ) {
      helpers.listen({
  ...
  requestDomain.run( function () {
    ...
    if ( !params.route.isStatic ) {
      ...
      if ( CTZN.patterns.controllers[CTZN.config.citizen.fallbackController || params.route.controller] && CTZN.patterns.controllers[CTZN.config.citizen.fallbackController || params.route.controller][params.route.action] ) {
        ...
        var error = {
              statusCode: 404,
              stack: new Error('404 - The requested controller action doesn\'t exist.\n\nController: ' + params.route.controller + '\nAction: ' + params.route.action + '()\n').stack
            };

        requestDomain.emit('error', error, params.request, params.response, params, context);
      }
    } else {
      ...
      if ( cachedFile ) {
        ...
        fs.readFile(staticPath, function (err, data) {
          if ( !err ) {
            ...
            requestDomain.emit('error', { statusCode: 404, staticAsset: true, file: params.route.filePath, stack: new Error('404 Not Found: '+ params.route.filePath).stack }, params.request, params.response, params, context);
          }
        });
      }
  ...
  responseDomain.run( function () {
    ...
    helpers.listen({
      ...
      if ( output.listen.success ) {
        ...
        server.emit('error', output.listen, params.request, params.response, params, context);
      }
    });
  });
}
