...
server.on('request', function (params, request, response, context) {
  ...
  if ( context.redirect && ( typeof context.redirect === 'string' || Object.keys(context.redirect).length ) && !params.route.direct ) {
    ...
    if ( CTZN.controllers.routes[controllerName]?.[params.route.action] ) {
...
server.on('error', async (params, request, response, context, err, respond = true) => {
  ...
  if ( params && !response.writableEnded ) {
    ...
    if ( respond ) {
      // Display the error view as long as the error didn't come from the error view itself.
      if ( params.route.chain[Object.keys(params.route.chain)[0]].controller !== 'error' ) {
      ...
      // If the error view throws an error, return that error along with the original error.
        ...
        response.write('<strong>Your error view threw an error (Irony +1):</strong><br><br><pre>' + err.toString() + '</pre><br><br><strong>This is the original error that caused the error handler to fire:</strong><br><br><pre>' + context.local.error.raw + '</pre>')
        response.end()
      }
    }
  } else {
...
async function fireController(params, request, response, context) {
  ...
  if ( proceed ) {
    ...
    // Set default local context to avoid server errors in the event of undefined variables
    ...
    response.setHeader('Cache-Control', context.headersLowercase?.['cache-control'] ? context.headersLowercase['cache-control'] : 'max-age=0')
    response.setHeader('ETag', context.headersLowercase?.['etag'] ? context.headersLowercase['etag'] : lastModified)
                ...
                includes[index] = ( async () => {
                  ...
                  if ( !params.route.chain[controller].include[item] ) {
                    ...
                    try {
                      ...
                      include[item].view = include[item].context.view || include[item].controller
      ...
      if ( CTZN.views[options.view] || CTZN.views[options.controller][options.view] ) {
        ...
        try {
          ...
          if ( CTZN.config.citizen.templateEngine === 'templateLiterals' ) {
            ...
            try {
              let consolidate = await import('consolidate')
