...
server.on('error', async (params, request, response, context, err, respond = true) => {
  ...
  if ( params && !response.writableEnded ) {
    ...
    if ( respond ) {
      ...
      if ( statusCode === 404 || params.route.chain[Object.keys(params.route.chain)[0]].controller !== 'error' ) {
      ...
      // If the error view throws an error, return that error along with the original error.
        ...
        let output = '<strong>Your error view threw an error (Irony +1):</strong><br><br><pre>' + err.toString() + '</pre>'

        if ( context.local?.error ) {
          output += '<br><br><strong>This is the original error that caused the error handler to fire:</strong><br><br><pre>' + context.local.error.raw + '</pre>'
        }
