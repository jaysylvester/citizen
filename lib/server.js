...
server.on('error', async (params, request, response, context, err, respond = true) => {
  ...
  helpers.log({
    type    : 'error',
    label   : label + ( statusCode >= 400 && statusCode < 500 ? ': ' + params.route.url + ( statusCode === 404 && request.headers.referer ? ' (Referrer: ' + request.headers.referer + ')' : '') : '\n'),
    content : logContent,
    divider : { top: statusCode >= 500, bottom: statusCode >= 500 }
  })
...
async function serve(request, response, protocol) {
  ...
  if ( !params.route.isStatic ) {
    ...
    let acceptFormat = request.headers['accept'] ? request.headers['accept'].split(',') : helpers.copy(CTZN.config.citizen.contentTypes),
        format = [],
        weight = 0
