                ...
                includes[index] = ( async () => {
                  ...
                  if ( typeof include[item] === 'string' ) {
                    ...
                    try {
                      ...
                      err.message = 'The requested include route (' + pathname + ') is invalid. When using the route syntax to specify the next controller in the chain, the route must be a valid pathname starting with a forward slash (/).'
                      server.emit('error', params, request, response, context, err)
                    }
                  } else {
...
function next(params, request, response, context) {
  if ( !response.writableEnded ) {
    ...
    if ( context.next && !params.route.direct ) {
      ...
      if ( CTZN.routes[context.next.controller]?.[context.next.action] ) {
