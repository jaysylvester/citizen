...
function createServer() {
  ...
  serverDomain.on('error', function (err) {
    ...
    switch ( err.code ) {
      ...
      case 'EADDRNOTAVAIL':
        helpers.log({
          content: '\nServer startup failed because the hostname you specified in your config file ("' + CTZN.config.citizen.hostname + '") appears to be in use by another process. Is there another web server running on this machine?\n',
          toConsole: true,
          file: 'citizen.txt',
          timestamp: false
        });
  ...
  serverDomain.run( function () {
    http.createServer( function (request, response) {
      ...
      var hostname = CTZN.config.citizen.hostname.length ? CTZN.config.citizen.hostname : '127.0.0.1',
          appUrl = CTZN.config.citizen.httpPort === 80 ? 'http://' + hostname + CTZN.config.citizen.urlPaths.app : 'http://' + hostname + ':' + CTZN.config.citizen.httpPort + CTZN.config.citizen.urlPaths.app,
      ...
      if ( !CTZN.config.citizen.hostname.length ) {
...
function fireController(params, context) {
  ...
  responseDomain.run( function () {
    ...
    helpers.listen({
      ...
      requestContext.handoffController = null;
      requestContext.handoffView = null;
      requestContext.handOffAction = null;
...
function handoff(params, requestContext) {
  ...
  if ( CTZN.patterns.views[lastLink.controller][lastLink.view] ) {
    ...
    if ( cachedController ) {
      ...
      lastLink.viewContent = renderView(lastLink.controller, lastLink.view, params.route.format, viewContext);
  ...
  requestContext.handoff = null;
  requestContext.view = null;
