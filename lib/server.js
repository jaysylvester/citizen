...
async function respond(params, request, response, context) {  
  ...
  for ( const link of Object.keys(params.route.chain) ) {
    ...
    if ( params.route.chain[link].params ) {
      ...
      parentContext = params.route.chain[link].context.content
      if ( params.route.chain[link].content && params.route.chain[link].content.include ) {
        ...
        for ( const include of Object.keys(params.route.chain[link].content.include) ) {
          ...
          if ( params.route.chain[link].content.include[include].render ) {
            ...
            parentContext.include[include] = params.route.chain[link].content.include[include].render
          } else {
            let includeContext = setViewContext(params, context, params.route.chain[link].content.include[include].context, params.route.chain[link].content.include[include].params)
            ...
            helpers.log({
              ...
              content: {
                controller  : params.route.chain[link].content.include[include].controller,
                action      : params.route.chain[link].content.include[include].action,
                view        : includeContext.context.view || params.route.chain[link].content.include[include].view
              }
            })
  
            parentContext.include[include] = await renderView({
            ...
            cacheController({
              ...
              context       : includeContext.context,
              render        : parentContext.include[include],
              params        : params.route.chain[link].content.include[include].params
            })
          }
        }
      ...
      linkContext = setViewContext(params, context, parentContext, params.route.chain[link].params)
  ...
  try {
    ...
    if ( CTZN.config.citizen.compression.enable && ( context.cache || ( request.client.encoding === 'gzip' || request.client.encoding === 'deflate' ) ) ) {
      ...
      if ( !response.writableEnded ) {
        response.setHeader('Content-Type', linkContext.contentType)
        response.setHeader('Content-Encoding', request.client.encoding)
        response.end(compressed[request.client.encoding] || render)
        server.emit('responseEnd', params, request, response, context)
      }
      cacheResponse(params, request, response, context, render, gzip, deflate, linkContext.contentType, lastModified)
    } else {
      if ( !response.writableEnded ) {
        response.setHeader('Content-Type', linkContext.contentType)
      ...
      cacheResponse(params, request, response, context, render, '', '', linkContext.contentType, lastModified)
    }
  } catch (err) {
    server.emit('error', params, request, response, context, err)
  }
...
function setViewContext(params, context, itemContext, itemParams) {
  let contentType, newContext

  switch ( params.route.format ) {
    case 'html':
      contentType = 'text/html'
      newContext = helpers.extend(itemContext, itemParams)
      if ( CTZN.config.citizen.mode === 'development' && ( CTZN.config.citizen.development.debug.view || params.url.ctzn_debug ) ) {
        newContext.ctzn_debugOutput = debug(params, context)
      }
      break
    case 'json':
      ...
      if ( !params.url.output ) {
        newContext = context.content
      } else {
        newContext = createJSON(params, context)
      }
      break
    case 'jsonp':
