...
server.on('sessionStart', async (params, request, response, context) => {
  ...
  if ( params.cookie.ctzn_sessionID && CTZN.sessions[params.cookie.ctzn_sessionID] && CTZN.sessions[params.cookie.ctzn_sessionID].expires > Date.now() ) {
    ...
    sessionID = session.create(params, request)
...
server.on('requestStart', async (params, request, response) => {
  let context = {}
  try {
    let requestStart = await CTZN.hooks.citizen.request.start(params, request)
  ...
  if ( context.redirect && ( typeof context.redirect === 'string' || Object.keys(context.redirect).length ) && !params.route.direct ) {
    ...
    if ( CTZN.patterns.controllers[CTZN.config.citizen.fallbackController || params.route.controller] && CTZN.patterns.controllers[CTZN.config.citizen.fallbackController || params.route.controller][params.route.action] ) {
      ...
      if ( request.headers.origin && !corsOriginTest.test(request.headers.origin) ) {
        ...
        if ( params.config.cors ) {
          params.config.corsLowerCase = {}
          Object.keys(params.config.cors).forEach( item => {
            // Create lowercase version of header name for easier comparison, while maintaining case for sent header
            params.config.corsLowerCase[item.toLowerCase()] = params.config.cors[item]
          })
          if ( params.config.corsLowerCase['access-control-allow-origin'] && ( params.config.corsLowerCase['access-control-allow-origin'].search(request.headers.origin) >= 0 || params.config.corsLowerCase['access-control-allow-origin'] === '*' ) ) {
            if ( request.method === 'OPTIONS' && !request.headers['access-control-request-method'] ) {
              respond = false
              response.end(server.emit('responseEnd', params, context))
            } else {
              Object.keys(params.config.cors).forEach( item => {
                response.setHeader(item, params.config.cors[item])
              })
              if ( params.config.cors['Access-Control-Allow-Origin'] ) {
                response.setHeader('Access-Control-Allow-Origin', request.headers.origin)
              } else {
                response.setHeader('access-control-allow-origin', request.headers.origin)
              }
              if ( params.config.corsLowerCase['access-control-allow-credentials'] ) {
                corsSession = true
              }
            }
          } else {
  ...
  if ( params && !response.writableEnded ) {
    ...
    switch ( params.route.format ) {
      case 'html':
        ...
        if ( CTZN.patterns.views.error ) {
          ...
          if ( CTZN.config.citizen.layout.controller.length && params.route.renderer !== CTZN.config.citizen.layout.controller ) {
            params.route.chain = params.route.chain.concat([{
              controller: 'error',
              action: '',
              view: errorView
            }])
            context.handoff = CTZN.config.citizen.layout
            preHandoff(params, request, response, context)
          // Otherwise, render the error view and send it to the client.
          } else {
          ...
          if ( err.stack ) {
            response.end('<pre><code>' + err.stack + '</code></pre>')
          } else {
            response.end('<pre><code>' + util.inspect(err) + '</code></pre>')
          }
        }
        break
      case 'json':
      case 'jsonp':
        ...
        response.setHeader('Content-Type', params.route.format === 'json' ? 'application/json' : 'text/javascript')
...
async function processRequest(params, request, response, context) {
  ...
  switch ( request.method ) {
    ...
    case 'POST':
      ...
      if ( params.config.form.maxFieldsSize ) {
        params.config.form.maxFieldsSize = params.config.form.maxFieldsSize * 1024
      }

      formParser = new formidable.IncomingForm(params.config.form)
...
async function fireController(params, request, response, context) {
  ...
  // Otherwise, try calling the controller.
    ...
    try {
      pattern = await CTZN.patterns.controllers[controller][action](helpers.copy(params), request, response) || {}
    } catch ( err ) {
      proceed = false
      server.emit('error', params, request, response, context, err)
    }
                ...
                includeProperties.forEach( function (item, index) {
                  ...
                  includes[index] = ( async () => {
                    if ( include[item].cache ) {
                      ...
                      try {
                        include[item].context = await CTZN.patterns.controllers[include[item].controller][include[item].action](helpers.copy(include[item].params), context) || {}
                      } catch (err) {
                        proceed = false
                      ...
                      if ( proceed ) {
                        switch ( include[item].params.route.format ) {
                          case 'html':
                            include[item].viewContext = Object.assign({}, include[item].context.content, include[item].params)
                            break
  ...
  if ( options.context.cache && options.context.cache.controller ) {
    ...
    if ( !cacheExists ) {
      ...
      if ( proceed ) {
        ...
        if ( options.context.legalFormat ) {
...
async function preHandoff(params, request, response, context) {
  var thisHandoff = Object.assign({}, context.handoff),
  ...
  if ( !CTZN.patterns.controllers[thisHandoff.controller] ) {
    ...
    server.emit('error', params, request, response, context, err)
  } else if ( params.route.format === 'html' && context.view !== false ) {
    if ( CTZN.patterns.views[lastLink.controller] && CTZN.patterns.views[lastLink.controller][lastLink.view] ) {
      cachedController = cache.getController({ controller: lastLink.controller, action: lastLink.action, view: lastLink.view, route: 'global' }) || cache.getController({ controller: lastLink.controller, action: lastLink.action, view: lastLink.view, route: params.route.pathname })
      ...
      if ( cachedController ) {
        ...
        handoff(params, request, response, context, thisHandoff, action, renderedView, renderer)
      } else {
        switch ( params.route.format ) {
          case 'html':
        ...
        try {
          helpers.log({
            label: 'Rendering view: ' + lastLink.view,
            content: renderLog
          })
      ...
      server.emit('error', {
        stack: new Error('HTML output requires a view template, but the handoff view you specified (' + lastLink.view + ') doesn\'t exist.').stack
      }, params, context)
    }
  } else {
    handoff(params, request, response, context, thisHandoff, action, renderedView, renderer)
...
function handoff(params, request, response, context, thisHandoff, action, renderedView, renderer) {
...
function extendConfig(params, controller) {
  if ( controller.config ) {
    ...
    if ( controller.config.global ) {
      params.config = helpers.extend(params.config, controller.config.global)
    }
    // Controller action config
    if ( controller.config[params.route.action] ) {
      params.config = helpers.extend(params.config, controller.config[params.route.action])
    }
  }
}
