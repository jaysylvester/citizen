...
server.on('error', function (err, params, context) {
  ...
  if ( CTZN.config.citizen.mode === 'production' || !debug(params, context) ) {
    logContent = err.stack ? requested + err.stack : requested + err
  } else {
    logContent = err.stack ? debug(params, context) + ' ' + requested + err.stack : debug(params, context) + ' ' + requested + err
  }
            ...
            if ( CTZN.patterns.views.error ) {
              ...
              helpers.listen({
                renderView: function (emitter) {
                  var viewContext = Object.assign({}, context.content, params);
                  renderView({ pattern: 'error', view: errorView, format: 'html', context: viewContext }, emitter);
                }
              }, function (output) {
            ...
            if ( output.listen.success ) {
              ...
              if ( routeCache ) {
                ...
                if ( params.request.headers['if-none-match'] === routeCache.lastModified ) {
                  ...
                  params.response.write(routeCache.render[params.client.encoding]);
                }
                params.response.end();
  ...
  if ( context.redirect && ( typeof context.redirect === 'string' || helpers.size(context.redirect) ) && params.route.type !== 'direct' ) {
    ...
    if ( params.request.headers.origin ) {
      ...
      if ( !corsOriginTest.test(params.request.headers.origin) ) {
        if ( controller.cors && controller.cors[params.route.action] ) {
          controller.corsLowerCase = {};
          controller.corsLowerCase[params.route.action] = {};
          for ( var property in controller.cors[params.route.action] ) {
            if ( controller.cors[params.route.action].hasOwnProperty(property) ) {
              ...
              controller.corsLowerCase[params.route.action][property.toLowerCase()] = controller.cors[params.route.action][property];
            }
          }
          if ( controller.corsLowerCase[params.route.action]['access-control-allow-origin'] && ( controller.corsLowerCase[params.route.action]['access-control-allow-origin'].search(params.request.headers.origin) >= 0 || controller.corsLowerCase[params.route.action]['access-control-allow-origin'] === '*' ) ) {
            if ( params.request.method === 'OPTIONS' && !params.request.headers['access-control-request-method'] ) {
              ...
              if ( controller.cors[params.route.action]['Access-Control-Allow-Origin'] ) {
  ...
  responseDomain.run( function () {
    ...
    helpers.listen({
      pattern: function (emitter) {
        ...
        // Stop waiting for a response if the client closes the connection
...
function cacheController(options) {
  ...
  if ( options.context.cache && options.context.cache.controller ) {
    ...
    if ( !cacheExists ) {
      contextParams = {
        // Intentionally leave out cookie and session params since those possibly contain sensitive data and shouldn't be cached
        client: options.params.client,
        config: options.params.config,
        form: options.params.form,
      ...
      switch ( options.format ) {
        case 'json':
          ...
          break;
        default:
          viewContext = Object.assign({}, cacheContext.content, contextParams);
      }
      ...
      if ( options.render ) {
        ...
        helpers.listen({
          renderView: function (emitter) {
            renderView({ pattern: options.controller,
              view: params.route.renderedView,
...
function respond(params, context) {
  ...
  helpers.listen({
    ...
    var renderView = output.renderView;
      ...
      if ( CTZN.config.citizen.compression.enable && ( context.cache || ( params.client.encoding === 'gzip' || params.client.encoding === 'deflate' ) ) ) {
        helpers.listen({
          ...
          if ( output.listen.success ) {
            ...
            params.response.end();
            server.emit('responseEnd', params, context);
            cacheResponse(params, context, renderView, output.gzip, output.deflate, contentType, lastModified);
          } else {
            ...
            params.response.write(renderView);
            params.response.end();
            server.emit('responseEnd', params, context);
        ...
        params.response.end();
        server.emit('responseEnd', params, context);
        cacheResponse(params, context, renderView, '', '', contentType, lastModified);
      }
    } else {
      server.emit('error', output.listen, params, context);
    }
