        ...
        case 'text/html':
          ...
          // Render only the error
            ...
            params.route.chain.error = {
              ...
              context: context,
              params: helpers.copy(params),
              render: '<pre>' + context.content.error.message + '</pre>'
            }
          }
          break
  ...
  for ( const link of Object.keys(params.route.chain) ) {
    ...
    if ( params.route.chain[link].context.view !== false ) {
      ...
      if ( params.route.chain[link].content && params.route.chain[link].content.include ) {
        params.route.chain[link].context.content.include = {}
        for ( const include of Object.keys(params.route.chain[link].content.include) ) {
          ...
          includes[includeInstance] = ( async () => {
            ...
            if ( params.route.chain[link].content.include[include].render ) {
              ...
              return params.route.chain[link].content.include[include].render
            } else {
              let includeContent
              try {
                includeContent = setViewContent(params, context, response.contentType, params.route.chain[link].content.include[include].context.content, params.route.chain[link].content.include[include].params)
              } catch ( err ) {
                return Promise.reject(err)
              }
              ...
              let render = await renderView(params, request, response, context, {
              ...
              if ( CTZN.config.citizen.cache.application.enable ) {
                try {
                  cacheController({
                    ...
                    contentType   : response.contentType,
                    context       : params.route.chain[link].content.include[include].context,
                    render        : render,
                    params        : params.route.chain[link].content.include[include].params
                  })
                } catch ( err ) {
      ...
      await Promise.allSettled(includes).then( async (results) => {
        results.forEach( (item, index) => {
          params.route.chain[link].context.content.include[includeNames[index]] = item.value
        })
        if ( params.route.chain[link].render ) {
          helpers.log({
          ...
          params.route.chain[link].viewContent = params.route.chain[link].render
        } else {
          let linkContent
          try {
            linkContent = setViewContent(params, context, response.contentType, params.route.chain[link].context.content, params.route.chain[link].params)
          } catch ( err ) {
          ...
          params.route.chain[link].viewContent = await renderView(params, request, response, context, {
          ...
          if ( CTZN.config.citizen.cache.application.enable ) {
            try {
              cacheController({
                ...
                render        : params.route.chain[link].viewContent,
                params        : params.route.chain[link].params
              })
            } catch ( err ) {
              server.emit('error', params, request, response, context, err, false)
  ...
  try {
    ...
    if ( CTZN.config.citizen.compression.enable && ( ( context.cache && context.cache.route ) || ( request.client.encoding === 'gzip' || request.client.encoding === 'deflate' ) ) ) {
      let [
        ...
        context.cache || request.client.encoding === 'deflate' ? compress.deflate(render) : '',
        context.cache || request.client.encoding === 'gzip'    ? compress.gzip(render)    : ''
      ]).catch(err => { server.emit('error', params, request, response, context, err) })
...
function setViewContent(params, context, contentType, itemContent, itemParams) {
  let viewContent

  switch ( contentType ) {
    case 'text/html':
      viewContent = helpers.extend(itemContent, itemParams)
      if ( CTZN.config.citizen.mode === 'development' && ( CTZN.config.citizen.development.debug.view || params.url.ctzn_debug ) ) {
        viewContent.ctzn_debugOutput = debug(params, context)
      }
      break
    case 'application/json':
      if ( !params.url.output ) {
        ...
        viewContent = createJSON(params, context)
      }
      break
    case 'application/javascript':
      if ( !params.url.output ) {
...
function cacheResponse(params, request, response, context, render, zipped, deflated, lastModified) {
  let cacheExists = cache.exists({
        route: params.route.pathname,
        contentType: response.contentType
      }),
...
async function renderView(params, request, response, context, options) {
  var viewContent = Object.assign({}, options.content), // consolidate modifies the original, so copy it
      json = '',
      callbackRegex
  ...
  switch ( response.contentType ) {
    case 'text/html':
      if ( CTZN.patterns.views[options.pattern][options.view] ) {
        if ( CTZN.config.citizen.mode === 'production' ) {
          viewContent.cache = CTZN.config.citizen.cache.application.enable
        }

        try {
          let html = await consolidate[CTZN.config.citizen.templateEngine](CTZN.patterns.views[options.pattern][options.view].path, viewContent)
          if ( options.content.ctzn_debugOutput ) {
            html = html.replace('</body>', '\n<div id="citizen-debug">\n' + options.content.ctzn_debugOutput + '\n</div>\n</body>')
          }
          return html
        } catch (err) {
    ...
    case 'application/json':
      ...
      if ( options.content.include ) {
        delete options.content.include
      }
      json = JSON.stringify(options.content, null, CTZN.config.citizen.mode === 'production' ? null : 2)
    ...
    case 'application/javascript':
      ...
      if ( callbackRegex.test(options.jsonpCallback) ) {
        ...
        json = options.jsonpCallback + '(' + JSON.stringify(options.content, null, CTZN.config.citizen.mode === 'production' ? null : 2) + ');'
      } else {
