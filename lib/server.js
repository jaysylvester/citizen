  ...
  switch ( CTZN.config.citizen.mode ) {
    case 'development':
      modeMessage += '\n' +
                     ...
                     '\n' +
                     '\n  To enable production mode, set your local NODE_ENV variable to "production"' +
                     '\n  or manually change the citizen config mode to "production".' +
                     '\n' +
...
async function serve(request, response, protocol) {
  ...
  if ( !params.route.isStatic ) {
    ...
    if ( CTZN.patterns.controllers[controllerName] && CTZN.patterns.controllers[controllerName][params.route.action] ) {
      // Extend the global config with the controller config
