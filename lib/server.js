  ...
  switch ( CTZN.config.citizen.mode ) {
    case 'production':
      if ( !err.staticAsset ) {
        ...
        helpers.log({
          label: '404 ' + http.STATUS_CODES[404],
          content: err.file,
          file: 'citizen.txt'
        });
      }
...
function createServer(options) {
  ...
  serverDomain.on('error', function (err) {
    ...
    switch ( err.code ) {
      ...
      case 'EADDRNOTAVAIL':
        helpers.log({
          content: '\n' + protocol + ' server startup failed because the hostname you specified in your config file ("' + hostname + '") appears to be in use by another process. Is there another web server running on this machine?\n',
          toConsole: true,
          file: 'citizen.txt',
          timestamp: false
        });
      ...
      if ( helpers.size(options.params.url) && options.context.cache.urlParams ) {
        Object.getOwnPropertyNames(options.params.url).forEach( function ( item, index, array) {
          if ( options.context.cache.urlParams.indexOf(item) < 0 ) {
            throw new Error('server.cacheController(): Invalid cache URL. The URL parameter [' + item + '] isn\'t permitted in a cached URL.');
          }
        });
      }
