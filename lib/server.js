  ...
  if ( params && !response.writableEnded ) {
    ...
    if ( respond ) {
      ...
      switch ( params.route.format ) {
        ...
        default:
          // Wipe the chain and context's content so only the error is rendered
          context.content = {
            error: error
          }
          params.route.chain = {
...
async function respond(params, request, response, context) {  
  ...
  for ( const link of Object.keys(params.route.chain) ) {
    ...
    parentContent = params.route.chain[link].context.content
    if ( params.route.chain[link].content && params.route.chain[link].content.include ) {
      ...
      for ( const include of Object.keys(params.route.chain[link].content.include) ) {
        ...
        if ( params.route.chain[link].content.include[include].render ) {
          ...
          parentContent.include[include] = params.route.chain[link].content.include[include].render
        } else {
          let includeContent
          try {
          ...
          if ( proceed ) {
            ...
            parentContent.include[include] = await renderView(params, request, response, context, {
            ...
            try {
              cacheController({
                ...
                render        : parentContent.include[include],
                params        : params.route.chain[link].content.include[include].params
              })
            } catch ( err ) {
    ...
    if ( proceed && params.route.chain[link].context.view !== false ) {
      if ( params.route.chain[link].render ) {
        ...
        try {
          linkContent = setViewContent(params, context, parentContent, params.route.chain[link].params)
        } catch ( err ) {
          proceed = false
          server.emit('error', params, request, response, context, err)
        }
  ...
  if ( proceed ) {
    ...
    try {
      ...
      if ( CTZN.config.citizen.compression.enable && ( context.cache || ( request.client.encoding === 'gzip' || request.client.encoding === 'deflate' ) ) ) {
        ...
        if ( !response.writableEnded ) {
          response.setHeader('Content-Type', linkContent.contentType)
          response.setHeader('Content-Encoding', request.client.encoding)
          response.end(compressed[request.client.encoding] || render)
          server.emit('responseEnd', params, request, response, context)
        }
        cacheResponse(params, request, response, context, render, gzip, deflate, linkContent.contentType, lastModified)
      } else {
        if ( !response.writableEnded ) {
          response.setHeader('Content-Type', linkContent.contentType)
        ...
        cacheResponse(params, request, response, context, render, '', '', linkContent.contentType, lastModified)
      }
    } catch (err) {
      server.emit('error', params, request, response, context, err)
    }
