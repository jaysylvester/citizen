...
server.on('request', function (params, request, response, context) {
  ...
  if ( context.redirect && ( typeof context.redirect === 'string' || Object.keys(context.redirect).length ) && !params.route.direct ) {
    ...
    if ( CTZN.patterns.controllers[controllerName] && CTZN.patterns.controllers[controllerName][params.route.action] ) {
      ...
      // set the response header to match the request header (per W3C recs). If not, throw
      // a 403 and end the response without rendering an error.
      if ( request.headers.origin && !corsOriginTest.test(request.headers.origin) ) {
        const failed = () => {
          response.end()
          respond = false
          let err = new Error('This request originated from a foreign host, but the controller action isn\'t configured for CORS requests from that host.')
          err.statusCode = 403
          server.emit('error', params, request, response, context, err)
        }
...
function createServer() {
  ...
  if ( CTZN.config.citizen.http.enabled ) {
    ...
    let appUrl = 'http://' + ( CTZN.config.citizen.http.hostname.length ? CTZN.config.citizen.http.hostname : '127.0.0.1' ) + ':' + ( CTZN.config.citizen.http.port === 80 ? '' : ':' + CTZN.config.citizen.http.port ) + CTZN.config.citizen.urlPath
  ...
  if ( CTZN.config.citizen.https.enabled ) {
    ...
    if ( startHttps ) {
      ...
      let appUrl = 'https://' + ( CTZN.config.citizen.https.hostname.length ? CTZN.config.citizen.https.hostname : '127.0.0.1' ) + ':' + ( CTZN.config.citizen.http.port === 443 ? '' : ':' + CTZN.config.citizen.http.port ) + CTZN.config.citizen.urlPath
...
async function serve(request, response, protocol) {
  let params = {
        route     : router.getRoute(request, protocol),
        url       : {},
        form      : {},
        payload   : {},
  ...
  let staticPath,
      cachedFile,
      compressable = CTZN.config.citizen.compression.enabled && !CTZN.config.citizen.compression.force && ( !params.route.isStatic || CTZN.config.citizen.compression.mimeTypes.indexOf(router.staticMimeTypes[params.route.extension]) >= 0 ),
      lastModified
                ...
                includeProperties.forEach( function (item, index) {
                  ...
                  includes[index] = ( async () => {
                    if ( !include[item].route ) {
                      ...
                      include[item].params.route = router.getRoute(request, include[item].params.route.protocol, include[item].route)
...
function buildCookie(params, request, cookies) {
  ...
  Object.keys(cookies).forEach( item => {
    ...
    if ( typeof cookies[item] === 'string' ) {
      ...
      cookieArray.push(item + '=' + cookies[item] + ';path=' + CTZN.config.citizen.urlPath + ';HttpOnly;SameSite=Lax;' + secure)
    } else {
      defaults = {
        value: '',
        path: CTZN.config.citizen.urlPath,
        expires: 'session',
        httpOnly: true,
        sameSite: request.cors ? 'None' : 'Lax',
        secure: secure
