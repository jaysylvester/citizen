...
server.on('error', async (params, request, response, context, err, respond = true) => {
  ...
  if ( statusCode >= 500 ) {
    if ( CTZN.config.citizen.mode === 'production' || !debug(params, request, response, context) ) {
      logContent = err.stack ? requested + err.stack : requested + util.inspect(err)
    } else {
      logContent = err.stack ? debug(params, request, response, context) + ' ' + requested + err.stack : debug(params, request, response, context) + ' ' + requested + util.inspect(err)
    }
  }
