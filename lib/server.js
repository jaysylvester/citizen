...
async function processRequest(params, request, response, context) {
  ...
  switch ( request.method ) {
    ...
    case 'POST':
      ...
      formParser.on('progress', function (bytesReceived, bytesExpected) {
        if ( bytesExpected > formParser.maxFieldsSize || bytesReceived > formParser.maxFieldsSize ) {
          let err = new Error('The maximum upload size for this form has been exceeded (max: ' + formParser.maxFieldsSize + ' bytes, received: ' + bytesReceived + ' bytes, expected: ' + bytesExpected + ' bytes).')
          err.statusCode = 413
          server.emit('error', params, request, response, context, err)
        }
      })
    ...
    case 'DELETE':
      switch ( response.contentType ) {
        // Handles text/plain, text/html, etc.
        default:
          ...
          request.on('end', function () {
            params.payload = body
            helpers.log({
              label: 'Payload received (' + response.contentType + ')',
        ...
        case 'application/json':
          ...
          request.on('end', function () {
            try {
              ...
              err.message = 'Failed to parse payload.'
              server.emit('error', params, request, response, context, err)
            }
            helpers.log({
              label: 'Payload received and parsed (' + response.contentType + ')',
              content: params.payload
            })
            server.emit('responseStart', params, request, response, context)
          })
...
function buildCookie(params, request, cookies) {
  ...
  Object.keys(cookies).forEach( item => {
    ...
    if ( typeof cookies[item] === 'string' ) {
      ...
      cookieArray.push(item + '=' + cookies[item] + ';path=/;HttpOnly;SameSite=Lax;' + secure)
    } else {
      defaults = {
        value: '',
        path: '/',
