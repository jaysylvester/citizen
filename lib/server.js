    ...
    helpers.listen({
      gzip: function (emitter) {
        if ( context.cache || params.client.encoding === 'gzip' ) {
          zlib.gzip(view, function (err, zippedFile) {
            if ( !err ) {
              emitter.emit('ready', zippedFile);
            } else {
      ...
      deflate: function (emitter) {
        if ( context.cache || params.client.encoding === 'deflate' ) {
          zlib.deflate(view, function (err, deflatedFile) {
            if ( !err ) {
              emitter.emit('ready', deflatedFile);
            } else {
              emitter.emit('error', err);
      ...
      if ( output.listen.success ) {
        ...
        params.response.setHeader('Content-Encoding', params.client.encoding);
        params.response.write(output[params.client.encoding] || view);
        params.response.end();
        server.emit('responseEnd', params, context);
        cacheResponse(params, context, view, output.gzip, output.deflate, contentType, lastModified, cacheControl);
      } else {
...
function cacheResponse(params, context, view, zippedView, deflatedView, contentType, lastModified, cacheControl) {
  if ( context.cache ) {
  ...
  var viewOutput = '',
    ...
    switch ( format ) {
      case 'html':
            ...
            case 'pug':
              switch ( CTZN.config.citizen.mode ) {
                default:
                  ...
                  break;
                case 'development':
                  viewOutput = fs.readFileSync(path.join(CTZN.config.citizen.directories.views, '/', pattern, '/', view + '.pug'), { 'encoding': 'utf8' });
                  viewOutput = CTZN.pug.compile(viewOutput, {
                ...
                case 'debug':
                  ...
                  viewOutput = CTZN.pug.compile(viewOutput, {
                    filename: CTZN.patterns.views[pattern][view].path,
                    pretty: true,
                    debug: CTZN.config.citizen.debug.pug
                  });
                  viewOutput = viewOutput(context);
                  break;
              }
        ...
        break;
      case 'json':
        viewOutput = JSON.stringify(context, null, CTZN.config.citizen.mode === 'production' ? null : 2);
      ...
      case 'jsonp':
        ...
        if ( callbackRegex.test(callback) ) {
          viewOutput = callback + '(' + JSON.stringify(context, null, CTZN.config.citizen.mode === 'production' ? null : 2) + ');';
        } else {
