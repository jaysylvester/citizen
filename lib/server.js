    ...
    httpDomain.run( function () {
      http.createServer( function (request, response) {
        ...
        var httpHostname = CTZN.config.citizen.http.hostname.length ? CTZN.config.citizen.http.hostname : '127.0.0.1',
            ...
            startupMessage = '\nThe HTTP server started successfully. Your app is running in ' + CTZN.config.citizen.mode + ' mode and ready to accept requests on port ' + CTZN.config.citizen.http.port + '. You can access it locally at:\n\n  ' + appUrl;
      ...
      if ( startHttps ) {
        https.createServer(httpsOptions, function (request, response) {
          ...
          var httpsHostname = CTZN.config.citizen.https.hostname.length ? CTZN.config.citizen.https.hostname : '127.0.0.1',
              ...
              startupMessage = '\nThe HTTPS server started successfully. Your secure app is running in ' + CTZN.config.citizen.mode + ' mode and ready to accept requests on port ' + CTZN.config.citizen.https.port + '. You can access it locally at:\n\n  ' + appUrl;
