...
async function serve(request, response, protocol) {
  let params = {
        route   : router.parseRoute(request, protocol),
        url     : {},
        form    : {},
        payload : {},
  ...
  request.remoteAddress = ( params ? request.headers['x-forwarded-for'] || request.socket.remoteAddress : 'undefined' ).replace('::ffff:', '')
