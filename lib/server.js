...
function processRequest(params, context) {
  ...
  if ( context.redirect && helpers.size(context.redirect) && params.route.type !== 'direct' ) {
  ...
  responseDomain.run( function () {
    ...
    helpers.listen({
      ...
      if ( output.listen.success ) {
        ...
        if ( requestContext.redirect && helpers.size(requestContext.redirect) && typeof requestContext.redirect.refresh === 'undefined' && params.route.type !== 'direct' ) {
          ...
          if ( requestContext.redirect && helpers.size(requestContext.redirect) && params.route.type !== 'direct' ) {
            redirect(params, requestContext);
          }
        ...
        context.error = {
          ...
          statusCode: output.listen.statusCode || 500,
          message: output.listen.message || 'server.fireController(): The ' + params.route.renderer + ' controller failed to emit a valid response.'
        };
        throw new Error(output.listen.message || 'server.fireController(): The ' + params.route.renderer + ' controller failed to emit a valid response.');
      }
    });
  });
}
