...
server.on('requestStart', async (params, request, response) => {
  ...
  process.on('unhandledRejection', (reason, promise) => {
    let err = new Error('Unhandled promise rejection: ' + promise + ', reason: ' + reason)
    throw err
  })
...
async function serve(request, response, protocol) {
  ...
  if ( !params.route.isStatic ) {
    ...
    if ( CTZN.patterns.controllers[controllerName] && CTZN.patterns.controllers[controllerName][params.route.action] ) {
      ...
      extendConfig(params, controllerName)
...
function handoff(params, request, response, context) {
  ...
  if ( CTZN.patterns.controllers[context.handoff.controller] ) {
    extendConfig(params, context.handoff.controller)
    fireController(params, request, response, context)
  } else {
  ...
  if ( refresh ) {
    ...
    if ( setReferrer ) {
      if ( CTZN.config.citizen.sessions.enabled ) {
        ...
        if ( context.cookies ) {
          context.cookies.ctzn_referer = ctzn_referer
        } else {
          context.cookies = {
            ctzn_referer: ctzn_referer
          }
...
function extendConfig(params, controllerName) {
  if ( CTZN.patterns.controllers[controllerName].config ) {
    // Controller config
    if ( CTZN.patterns.controllers[controllerName].config.controller ) {
      params.config.citizen = helpers.extend(params.config.citizen, CTZN.patterns.controllers[controllerName].config.controller)
    }
    // Controller action config
    if ( CTZN.patterns.controllers[controllerName].config[params.route.action] ) {
      params.config.citizen = helpers.extend(params.config.citizen, CTZN.patterns.controllers[controllerName].config[params.route.action])
    }
  }
}
