  ...
  if ( context.redirect && ( typeof context.redirect === 'string' || Object.keys(context.redirect).length ) && !params.route.direct ) {
    ...
    if ( CTZN.patterns.controllers[CTZN.config.citizen.fallbackController || params.route.controller] && CTZN.patterns.controllers[CTZN.config.citizen.fallbackController || params.route.controller][params.route.action] ) {
      let controller = CTZN.patterns.controllers[params.route.controller],
          corsOriginTest = new RegExp('^' + ( params.request.headers['x-forwarded-proto'] || params.route.protocol ) + '://' + ( params.request.headers['x-forwarded-host'] || params.request.headers.host ) + '$'),
          corsSession = false,
          respond = true
      ...
      if ( params.request.headers.origin && !corsOriginTest.test(params.request.headers.origin) ) {
        ...
        if ( params.config.cors ) {
          ...
          if ( params.config.corsLowerCase['access-control-allow-origin'] && ( params.config.corsLowerCase['access-control-allow-origin'].search(params.request.headers.origin) >= 0 || params.config.corsLowerCase['access-control-allow-origin'] === '*' ) ) {
            if ( params.request.method === 'OPTIONS' && !params.request.headers['access-control-request-method'] ) {
              ...
              Object.keys(params.config.cors).forEach( item => {
                params.response.setHeader(item, params.config.cors[item])
              })
...
async function serve(request, response, protocol) {
  ...
  if ( !params.route.isStatic ) {
    ...
    server.emit('requestStart', params)
  } else {
            ...
            if ( !params.response.headersSent ) {
              ...
              if ( request.headers['if-none-match'] == lastModified ) {
                ...
                if ( CTZN.config.citizen.compression.enable && compressable ) {
                  ...
                  ]).catch(err => { server.emit('error', err, params) })
...
async function processRequest(params, context) {
  ...
  switch ( params.request.method ) {
    ...
    case 'POST':
      ...
      formParser.parse(params.request, async (err, fields, files) => {
        if ( !err ) {
              ...
              if ( includeProperties.length && params.route.format === 'html' ) {
                includeProperties.forEach( function (item, index) {
                  ...
                  if ( !include[item].route ) {
                    ...
                    include[item].params.route = router.getRoute(include[item].params.route.protocol + '://' + params.request.headers.host + include[item].route)
...
function redirect(params, context, setReferrer) {
  var url = typeof context.redirect === 'string' ? context.redirect : context.redirect.url,
      ...
      ctzn_referer = ( ( params.request.headers['x-forwarded-proto'] || params.route.protocol ) + '://' ) + ( params.request.headers['x-forwarded-host'] || params.request.headers.host ) + params.route.pathname
...
function buildCookie(cookies, params) {
  var defaults = {},
      ...
      sameSite = '',
      secure = '',
      secureEnabled = ( params.request.headers['x-forwarded-proto'] === 'https' || params.route.protocol === 'https' ) && CTZN.config.citizen.https.secureCookies,
      cookieExpires,
      now = Date.now()
