...
function respond(params, requestContext, renderer, renderedView, include, includeProperties) {
  ...
  switch ( params.route.format ) {
    case 'html':
      ...
      if ( params.route.name !== params.route.renderer && CTZN.patterns.views[params.route.name] && CTZN.patterns.views[params.route.name][params.route.view] ) {
        viewContext.include[params.route.name] = renderView(params.route.name, params.route.view, 'html', viewContext);
      }
