...
async function fireController(params, request, response, context) {
  ...
  if ( proceed ) {
    ...
    // Set the link's params to this controller's params
    ...
    // If it's a direct controller request, set the JSON output
...
function createJSON(node, context) {
  let outputArray = node.split('~'),
      ...
      output = {},
      failed = () => {
        let err = new Error('The requested JSON notation ("' + node + '") doesn\'t exist. Make sure you\'re using a tilde (~) as a delimiter for nested output.')
        err.statusCode = 404
        throw err
      }
