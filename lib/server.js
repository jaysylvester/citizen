...
async function fireController(params, request, response, context) {
  ...
  if ( proceed ) {
    ...
    // If the handoff directive isn't specified, it's the last controller in
    // the chain, so check for a layout controller and hand off if it exists.
                ...
                includeProperties.forEach( function (item, index) {
                  ...
                  includes[index] = ( async () => {
                    if ( include[item].cache ) {
                      // Append the include's public context to the calling controller's public context and the overall context
                      params.route.chain[controller].context.public.include[item] = include[item].cache.context.public
                      context.public.include[item] = include[item].cache.context.public
                      ...
                      try {
                        ...
                        context.public.include[item] = include[item].context.public
        ...
        for ( const include of Object.keys(params.route.chain[link].include) ) {
          ...
          includes[includeInstance] = ( async () => {
            ...
            if ( params.route.chain[link].include[include].output ) {
              ...
              let publicContext
              try {
                publicContext = setPublicContext(params, context, response.contentType, params.route.chain[link].include[include].context.public, params.route.chain[link].include[include].params)
              } catch ( err ) {
                return Promise.reject(err)
              }
...
function setPublicContext(params, context, contentType, controllerPublicContext, controllerParams) {
  let publicContext = {}
  ...
  switch ( contentType ) {
    ...
    case 'application/json':
      if ( !params.url.output ) {
        publicContext = helpers.extend(controllerPublicContext, context.public)
      } else {
        publicContext = createJSON(params, helpers.extend(controllerPublicContext, context.public))
      }
      break
    case 'application/javascript':
      if ( !params.url.output ) {
