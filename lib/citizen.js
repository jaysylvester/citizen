...
async function getAppOn() {
  ...
  await files.forEach( async (file) => {
    if ( jsRegex.test(file) ) {
      ...
      on[path.basename(file, path.extname(file))] = await import(path.join(finalConfig.citizen.directories.hooks, '/', file))
    }
  })
...
async function getPatterns() {
  ...
  await controllers.forEach( async (file) => {
    if ( jsRegex.test(file) ) {
      console.log('  ' + finalConfig.citizen.directories.controllers + '/' + file)
      patterns.controllers[path.basename(file, path.extname(file))] = await import(path.join(finalConfig.citizen.directories.controllers, '/', file))
    }
  })
  ...
  if ( models.length > 0 ) {
    ...
    await models.forEach( async (file) => {
      if ( jsRegex.test(file) ) {
        ...
        patterns.models[path.basename(file, path.extname(file))] = await import(path.join(finalConfig.citizen.directories.models, '/', file))
      }
    })
  } else {
...
import * as serverExp from './server.js'
