...
const getRoute = (request, protocol, pathname) => {
  let urlToParse = ( ( request.headers['x-forwarded-proto'] || protocol ) + '://' ) + ( request.headers['x-forwarded-host'] || request.headers.host ) + ( pathname || request.url ),
      ...
      route = {
        parsed        : parsed,
        base          : parsed.protocol + '//' + parsed.host + CTZN.config.citizen.urlPath,
        isStatic      : false,
  ...
  if ( !staticMimeTypes[pathToParse.replace(staticRegex, '$1')] ) {
    if ( CTZN.config.citizen.urlPath !== '/' ) {
      pathToParse = pathToParse.replace(CTZN.config.citizen.urlPath, '')
    }
    ...
    if ( CTZN.config.citizen.urlPath !== '/' && route.filePath.indexOf(CTZN.config.citizen.urlPath) === 0 ) {
      route.filePath = route.filePath.replace(CTZN.config.citizen.urlPath, '')
    }
  }
