...
function set(options) {
  var timer,
      ...
      value,
      stats,
      enableCache = ( CTZN.config.citizen.mode !== 'debug' && CTZN.config.citizen.cache.enable ) || ( CTZN.config.citizen.mode === 'debug' && !CTZN.config.citizen.debug.disableCache && CTZN.config.citizen.cache.enable ) ? true : false;
  ...
  if ( enableCache ) {
    ...
    if ( options.lifespan !== 'application' && !helpers.isNumeric(options.lifespan) ) {
      throw new Error('cache.set(): Cache lifespan needs to be specified in milliseconds.');
    }
