...
const setRoute = (options) => {
  ...
  if ( !CTZN.cache.routes[options.route] || !CTZN.cache.routes[options.route][options.contentType] ) {
    ...
    // Don't cache the controller cache directive or it will re-cache the route on every request. We keep the request cache directive intact so calls to cached controllers can still cache the request.
    delete options.context.cache.controller
    // Don't cache the include directive because cached controller views already contain the rendered include and its local scope
...
const getRoute = (options) => {
  if ( CTZN.cache.routes && CTZN.cache.routes[options.route] && CTZN.cache.routes[options.route][options.contentType] ) {
    ...
    // Route caches are stored as buffers, so they can't be copied prior to output.
    // This is fine since server.js doesn't assign the output to a pointer that
    // receives additions or modifications.
...
export default { clear, exists, get, getRoute, set, setRoute, setController }
