...
const set = (options) => {
  let timer,
      ...
      enableCache = options.file ? CTZN.config.citizen.cache.static.enable : CTZN.config.citizen.cache.application.enable
...
const exists = (options) => {
  ...
  // If only a route is provided, check if the route scope has it
    ...
    if ( CTZN.cache.routes && CTZN.cache.routes[options.route] && Object.keys(CTZN.cache.routes[options.route]).length ) {
      return true
    }
